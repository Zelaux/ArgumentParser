...
import argumentparser.struct.Pool;
import org.intellij.lang.annotations.Language;

import java.util.ArrayList;
import java.util.List;

public class ParamPatternParser {
    static final ThreadLocal<Pool<TextRegion>> textRegionPool = ThreadLocal.withInitial(()->new Pool<>(10,TextRegion::new));
    ...
    public static int getParamAmount(@Language("ExtendedArcCommandParams") String text) {
        List<TextRegion> tmpRegions = state.get().tmpRegions;
    ...
    public static ParamPattern parse(@Language("ExtendedArcCommandParams") String text) throws ParamPatternParseException {
        final List<TextRegion> tmpRegions = state.get().tmpRegions;
        collectRegions(text, tmpRegions, true);
        Param[] params = new Param[tmpRegions.size()];
        ...
        clear(tmpRegions);
        return new ParamPattern(params);
    }

    private static int collectRegions(String text, final List<TextRegion> tmpRegions, boolean canThrow) {
        clear(tmpRegions);
    ...
    private static void clear(final List<TextRegion> tmpRegions) {
        Pool<TextRegion> pool = textRegionPool.get();
        pool.size=0;
        tmpRegions.clear();
    }

    private static byte completeParam(String text, final List<TextRegion> tmpRegions, int begin, int end, int handlerBegin, int handleEnd, boolean canThrow) {
        if (end - begin <= 2) {
            if (canThrow) {
    ...
    private static TextRegion textRegion(int begin, int end, int handlerBegin, int handleEnd) {
        return textRegionPool.get().get().set(begin, end, handlerBegin, handleEnd);
    }

    static void throwException(String message, int startIndex, int endIndex, String rawText) {
        throw new ParamPatternParseException(message, startIndex, endIndex, rawText);
    }
    ...
    public static final class CommandParamParserState {
        final List<TextRegion> tmpRegions = new ArrayList<>();
    }

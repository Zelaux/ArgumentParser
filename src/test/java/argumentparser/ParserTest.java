...
import zelauxtests.TestUtils;

public class ParserTest extends TestCase {
    protected static void checkError(String message, Runnable runnable) {
        //noinspection unchecked
        TestUtils.checkError(message, new Class[]{ParamPatternParseException.class}, runnable);
    }
    ...
    public void testParsingVariadic() {
        Assert.assertArrayEquals(arr(new Param("it", true, true, null)), ParamPatternParser.parse("[it...]").params);
    }
    @Test
    public void testHandlers() {
        Assert.assertArrayEquals(arr(new Param("it(some)", true, true, "some")), ParamPatternParser.parse("[it(some)...]").params);
        Assert.assertArrayEquals(arr(new Param("it(some)", true, true, "some")), ParamPatternParser.parse("[it...(some)]").params);
    }
}
